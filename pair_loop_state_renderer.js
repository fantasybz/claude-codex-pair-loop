...
function main() {
  ...
  const data = {
    ...
    validation: {
      configuredCommand: process.env.VALIDATION_COMMAND || "",
      commandUsed: process.env.VALIDATION_COMMAND_USED || "",
      preset: process.env.VALIDATION_PRESET_EFFECTIVE || "auto",
      selectionMode: process.env.VALIDATION_SELECTION_MODE || "auto",
      detected: {
        ecosystem: process.env.VALIDATION_DETECTED_ECOSYSTEM || null,
        layout: process.env.VALIDATION_DETECTED_LAYOUT || null,
        reason: process.env.VALIDATION_DETECTED_REASON || null,
        suggestedCommand: process.env.VALIDATION_DETECTED_COMMAND || null,
      },
      warning: process.env.VALIDATION_PRECHECK_WARNING || null,
      hint: process.env.VALIDATION_PRECHECK_HINT || null,
    },
    fileFocus,
    openDecisions,
  ...
  const markdown = [
    ...
    "",
    "## Validation Details",
    `- Preset: ${data.validation.preset}`,
    `- Selection mode: ${data.validation.selectionMode}`,
    `- Configured command: ${data.validation.configuredCommand || "auto"}`,
    `- Command used: ${data.validation.commandUsed || "none"}`,
    `- Detected layout: ${data.validation.detected.layout || "unknown"}`,
    `- Detection reason: ${data.validation.detected.reason || "none"}`,
    `- Suggested command: ${data.validation.detected.suggestedCommand || "none"}`,
    `- Warning: ${data.validation.warning || "none"}`,
    `- Hint: ${data.validation.hint || "none"}`,
    "",
    "## File Focus",
    data.fileFocus,
    "",
