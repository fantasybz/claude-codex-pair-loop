---
name: claude-first-pair-loop
description: Run the local Claude Code and Codex pair-loop tools from this repository with Claude Code taking the first turn. Use when Claude Code should start, resume, or continue an iterative pair-programming loop for a coding task, and the desired turn order is Claude-first. Supports both standard mode and MCP mode, plus preserve and resume flags.
---
...
Launch the repository pair-loop scripts with the turn order fixed to Claude-first. Prefer this skill when the user wants Claude Code to lead each iteration and Codex to respond second.
...
Use the wrapper script so the skill always enforces Claude-first order.
...
```bash
./skills/claude-first-pair-loop/scripts/run-pair-loop.sh --mcp --task "Build a CLI tool" --max-iterations 3
```
